...
def _save_uploaded_files(uploaded_files, run_id: str) -> list[str]:
    """Write Streamlit's in-memory uploaded files to a run-scoped folder
    under data/landing/ and return their paths -- every agent works with
    file paths, not in-memory buffers. Scoping by run_id means two runs
    uploading a same-named file (e.g. two people both using sales_data.csv)
    can't overwrite each other mid-pipeline -- the Bronze Agent re-reads
    these paths from disk when it actually executes, not just at upload
    time, so a later run's upload could otherwise silently corrupt an
    earlier run still waiting at a HITL approval gate."""
    run_landing_dir = LANDING_DIR / run_id[:8]
    run_landing_dir.mkdir(parents=True, exist_ok=True)

    saved_paths = []
    for uploaded_file in uploaded_files:
        dest = run_landing_dir / uploaded_file.name
...
if state is None:
    ...
    if st.button("\U0001F680 Start Pipeline", type="primary", disabled=not (uploaded_files and business_intent)):
        st.session_state.retry_action = "phase_1"
        with st.spinner("Profiling your data and drafting Bronze layer rules..."):
            run_id = str(uuid.uuid4())
            file_paths = _save_uploaded_files(uploaded_files, run_id)
            st.session_state.pipeline_state = orchestrator.start_pipeline(
                file_paths, business_intent, run_id=run_id
            )
        st.rerun()
...
# --- A run is in progress or finished ---
    ...
    if state.status == "failed":
        ...
        with open(state.report_path, "r", encoding="utf-8") as f:
            report_html = f.read()
        # This report is generated by our own pipeline. It does embed the
        # user's business_intent and the LLM's summary text, but reporter.py
        # HTML-escapes both before writing the file, so it's safe to embed
        # here even though st.iframe() executes any script in its content.
